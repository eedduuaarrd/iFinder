export type Lang = "ca" | "es" | "en";

export const LANGS: { code: Lang; label: string; flag: string }[] = [
  { code: "ca", label: "Català", flag: "🏴" },
  { code: "es", label: "Castellano", flag: "🇪🇸" },
  { code: "en", label: "English", flag: "🇬🇧" },
];

type Dict = Record<string, string>;

const en: Dict = {
  "common.loading": "LOADING...",
  "common.error": "ERROR",
  "common.retry": "RETRY",
  "common.cancel": "CANCEL",
  "common.save": "SAVE",
  "common.continue": "CONTINUE",
  "common.back": "BACK",
  "common.points": "PTS",

  "nav.hunt": "HUNT",
  "nav.ranks": "RANKS",
  "nav.crew": "CREW",

  "auth.title": "iFinder",
  "auth.tagline": "Street Scavenger Hunt // Drop In",
  "auth.username": "Username",
  "auth.email": "Email",
  "auth.password": "Password",
  "auth.city": "City",
  "auth.cityPlaceholder": "Your town/city",
  "auth.cityHint": "Used for the local leaderboard",
  "auth.signIn": "ENTER",
  "auth.signUp": "JOIN THE HUNT",
  "auth.processing": "PROCESSING...",
  "auth.toLogin": "Already initiated? Log in",
  "auth.toSignUp": "New blood? Sign up",
  "auth.fail": "Authentication failed",
  "auth.passwordTooShort": "Password must be at least 6 characters",
  "auth.usernameTooShort": "Username must be at least 3 characters",
  "auth.invalidEmail": "Invalid email address",
  "auth.emailInUse": "This email is already registered",
  "auth.usernameTaken": "This username is already taken",
  "auth.invalidLogin": "Wrong email or password",
  "auth.networkError": "Network error. Please try again.",
  "auth.show": "Show",
  "auth.hide": "Hide",
  "auth.passwordHint": "At least 6 characters",
  "auth.usernameHint": "Letters, numbers, _ — min 3 chars",

  "hunt.title": "THIS WEEK",
  "hunt.subtitle": "10 TARGETS // FIND THEM",
  "hunt.empty": "NO ACTIVE HUNT",
  "hunt.found": "FOUND",
  "hunt.progress": "PROGRESS",
  "hunt.points": "POINTS",
  "hunt.streak": "STREAK",
  "hunt.streakDays": "DAYS",
  "hunt.daily": "DAILY 2x",
  "hunt.dailyDesc": "First hunt today: DOUBLE points!",
  "hunt.combo": "COMBO",
  "hunt.comboDesc": "Catch 2 in 5 min for +50%",
  "hunt.timeLeft": "ENDS IN",
  "hunt.completed": "WEEK CLEARED!",
  "hunt.rank": "YOUR RANK",
  "hunt.others": "OTHERS FOUND",
  "hunt.tap": "TAP TO HUNT",

  "friends.title": "THE CREW",
  "friends.search": "SEARCH USERNAMES...",
  "friends.searchResults": "SEARCH RESULTS",
  "friends.pending": "PENDING REQUESTS",
  "friends.allies": "ALLIES",
  "friends.alone": "YOU HUNT ALONE.",
  "friends.findFriends": "FIND SOME FRIENDS.",
  "friends.add": "ADD",
  "friends.weekPts": "WK PTS",
  "friends.foundLabel": "FOUND:",
  "friends.requestSent": "REQUEST SENT",
  "friends.requestSentDesc": "Friend request dispatched.",
  "friends.accepted": "FRIEND ADDED",
  "friends.acceptedDesc": "You are now allies.",

  "rankings.title": "LEADERBOARD",
  "rankings.local": "LOCAL",
  "rankings.global": "GLOBAL",
  "rankings.friends": "CREW",
  "rankings.empty": "NO DATA YET",
  "rankings.foundShort": "FOUND",
  "rankings.you": "YOU",

  "profile.title": "DOSSIER",
  "profile.logout": "LOGOUT",
  "profile.totalPts": "TOTAL PTS",
  "profile.weekPoints": "WEEK POINTS",
  "profile.globalRank": "GLOBAL RANK",
  "profile.weeklyMosaic": "WEEKLY MOSAIC",
  "profile.share": "SHARE",
  "profile.noCaptures": "NO CAPTURES THIS WEEK",
  "profile.language": "LANGUAGE",
  "profile.settings": "SETTINGS",
  "profile.editCity": "City",
  "profile.cityHint": "Used for the local leaderboard",
  "profile.saveSettings": "SAVE",
  "profile.saved": "Saved!",
  "profile.saveError": "Could not save changes",
  "profile.badges": "BADGES",
  "profile.badge.first": "FIRST CATCH",
  "profile.badge.streak": "ON FIRE",
  "profile.badge.completer": "COMPLETIONIST",
  "profile.badge.speedster": "SPEEDSTER",
  "profile.badge.social": "SOCIALITE",
  "profile.locked": "LOCKED",

  "camera.target": "TARGET",
  "camera.color": "COLOR",
  "camera.frame": "FRAME THE TARGET. ONE SHOT.",
  "camera.verified": "VERIFIED BY AI",
  "camera.found": "TARGET FOUND",
  "camera.confidence": "CONFIDENCE",
  "camera.continue": "CONTINUE HUNT →",
  "camera.rejected": "AI REJECTED THIS",
  "camera.notValid": "NOT VALID",
  "camera.giveUp": "GIVE UP",
  "camera.alreadyCaught": "ALREADY CAUGHT THIS ONE",
  "camera.cameraDenied": "CAMERA ACCESS DENIED. ENABLE IT IN YOUR BROWSER.",
  "camera.goBack": "GO BACK",
  "camera.aiActive": "AI VISION ACTIVE",
  "camera.scan1": "SCANNING FRAME...",
  "camera.scan2": "DETECTING OBJECTS...",
  "camera.scan3": "VERIFYING TARGET...",
  "camera.scan4": "MEASURING CONFIDENCE...",
  "camera.bonus": "SPEED BONUS!",
};

const ca: Dict = {
  "common.loading": "CARREGANT...",
  "common.error": "ERROR",
  "common.retry": "TORNA-HI",
  "common.cancel": "CANCEL·LA",
  "common.save": "DESA",
  "common.continue": "CONTINUA",
  "common.back": "ENRERE",
  "common.points": "PTS",

  "nav.hunt": "CAÇA",
  "nav.ranks": "RÀNQUING",
  "nav.crew": "COLLA",

  "auth.title": "iFinder",
  "auth.tagline": "Caça d'objectes urbana // Entra",
  "auth.username": "Nom d'usuari",
  "auth.email": "Correu",
  "auth.password": "Contrasenya",
  "auth.city": "Ciutat",
  "auth.cityPlaceholder": "El teu poble/ciutat",
  "auth.cityHint": "Per al rànquing local",
  "auth.signIn": "ENTRA",
  "auth.signUp": "UNEIX-T'HI",
  "auth.processing": "PROCESSANT...",
  "auth.toLogin": "Ja tens compte? Entra",
  "auth.toSignUp": "Nou? Registra't",
  "auth.fail": "Error d'autenticació",
  "auth.passwordTooShort": "La contrasenya ha de tenir mínim 6 caràcters",
  "auth.usernameTooShort": "El nom d'usuari ha de tenir mínim 3 caràcters",
  "auth.invalidEmail": "Correu no vàlid",
  "auth.emailInUse": "Aquest correu ja està registrat",
  "auth.usernameTaken": "Aquest nom d'usuari ja està en ús",
  "auth.invalidLogin": "Correu o contrasenya incorrectes",
  "auth.networkError": "Error de connexió. Torna-ho a provar.",
  "auth.show": "Mostra",
  "auth.hide": "Amaga",
  "auth.passwordHint": "Mínim 6 caràcters",
  "auth.usernameHint": "Lletres, números, _ — mín 3 caràcters",

  "hunt.title": "AQUESTA SETMANA",
  "hunt.subtitle": "10 OBJECTIUS // TROBA'LS",
  "hunt.empty": "CAP CAÇA ACTIVA",
  "hunt.found": "TROBAT",
  "hunt.progress": "PROGRÉS",
  "hunt.points": "PUNTS",
  "hunt.streak": "RATXA",
  "hunt.streakDays": "DIES",
  "hunt.daily": "BONUS DIARI 2x",
  "hunt.dailyDesc": "Primera caça d'avui: punts DOBLES!",
  "hunt.combo": "COMBO",
  "hunt.comboDesc": "Caça 2 en 5 min: +50%",
  "hunt.timeLeft": "ACABA EN",
  "hunt.completed": "SETMANA COMPLETADA!",
  "hunt.rank": "EL TEU RANG",
  "hunt.others": "ALTRES HAN TROBAT",
  "hunt.tap": "TOCA PER CAÇAR",

  "friends.title": "LA COLLA",
  "friends.search": "BUSCA USUARIS...",
  "friends.searchResults": "RESULTATS",
  "friends.pending": "SOL·LICITUDS PENDENTS",
  "friends.allies": "AMICS",
  "friends.alone": "CACES TOT SOL.",
  "friends.findFriends": "BUSCA AMICS.",
  "friends.add": "AFEGEIX",
  "friends.weekPts": "PTS SETM",
  "friends.foundLabel": "TROBAT:",
  "friends.requestSent": "ENVIADA",
  "friends.requestSentDesc": "Sol·licitud d'amistat enviada.",
  "friends.accepted": "AMIC AFEGIT",
  "friends.acceptedDesc": "Ja sou amics.",

  "rankings.title": "RÀNQUING",
  "rankings.local": "LOCAL",
  "rankings.global": "GLOBAL",
  "rankings.friends": "AMICS",
  "rankings.empty": "ENCARA NO HI HA DADES",
  "rankings.foundShort": "TROBATS",
  "rankings.you": "TU",

  "profile.title": "PERFIL",
  "profile.logout": "SURT",
  "profile.totalPts": "PTS TOTALS",
  "profile.weekPoints": "PTS SETMANA",
  "profile.globalRank": "RANG GLOBAL",
  "profile.weeklyMosaic": "MOSAIC SETMANAL",
  "profile.share": "COMPARTEIX",
  "profile.noCaptures": "CAP CAPTURA AQUESTA SETMANA",
  "profile.language": "IDIOMA",
  "profile.settings": "CONFIGURACIÓ",
  "profile.editCity": "Ciutat",
  "profile.cityHint": "S'utilitza per al rànquing local",
  "profile.saveSettings": "DESA",
  "profile.saved": "Desat!",
  "profile.saveError": "No s'han pogut desar els canvis",
  "profile.badges": "MEDALLES",
  "profile.badge.first": "PRIMERA CAÇA",
  "profile.badge.streak": "EN RATXA",
  "profile.badge.completer": "COMPLETISTA",
  "profile.badge.speedster": "VELOCISTA",
  "profile.badge.social": "SOCIABLE",
  "profile.locked": "BLOQUEJAT",

  "camera.target": "OBJECTIU",
  "camera.color": "COLOR",
  "camera.frame": "ENQUADRA L'OBJECTIU. UN TIR.",
  "camera.verified": "VERIFICAT PER IA",
  "camera.found": "OBJECTIU TROBAT",
  "camera.confidence": "CONFIANÇA",
  "camera.continue": "SEGUEIX CAÇANT →",
  "camera.rejected": "LA IA HO HA REBUTJAT",
  "camera.notValid": "NO VÀLID",
  "camera.giveUp": "DEIXA-HO",
  "camera.alreadyCaught": "JA HAS CAÇAT AQUEST OBJECTIU",
  "camera.cameraDenied": "ACCÉS A LA CÀMERA DENEGAT. ACTIVA'L AL NAVEGADOR.",
  "camera.goBack": "TORNA",
  "camera.aiActive": "VISIÓ IA ACTIVA",
  "camera.scan1": "ESCANEJANT IMATGE...",
  "camera.scan2": "DETECTANT OBJECTES...",
  "camera.scan3": "VERIFICANT OBJECTIU...",
  "camera.scan4": "CALCULANT CONFIANÇA...",
  "camera.bonus": "BONUS DE VELOCITAT!",
};

const es: Dict = {
  "common.loading": "CARGANDO...",
  "common.error": "ERROR",
  "common.retry": "REINTENTAR",
  "common.cancel": "CANCELAR",
  "common.save": "GUARDAR",
  "common.continue": "CONTINUAR",
  "common.back": "ATRÁS",
  "common.points": "PTS",

  "nav.hunt": "CAZA",
  "nav.ranks": "RÁNKING",
  "nav.crew": "GRUPO",

  "auth.title": "iFinder",
  "auth.tagline": "Caza urbana de objetos // Entra",
  "auth.username": "Nombre de usuario",
  "auth.email": "Correo",
  "auth.password": "Contraseña",
  "auth.city": "Ciudad",
  "auth.cityPlaceholder": "Tu pueblo/ciudad",
  "auth.cityHint": "Para el ranking local",
  "auth.signIn": "ENTRAR",
  "auth.signUp": "UNIRSE",
  "auth.processing": "PROCESANDO...",
  "auth.toLogin": "¿Ya tienes cuenta? Entra",
  "auth.toSignUp": "¿Nuevo? Regístrate",
  "auth.fail": "Error de autenticación",
  "auth.passwordTooShort": "La contraseña debe tener mínimo 6 caracteres",
  "auth.usernameTooShort": "El nombre de usuario debe tener mínimo 3 caracteres",
  "auth.invalidEmail": "Correo no válido",
  "auth.emailInUse": "Este correo ya está registrado",
  "auth.usernameTaken": "Este nombre de usuario ya está en uso",
  "auth.invalidLogin": "Correo o contraseña incorrectos",
  "auth.networkError": "Error de conexión. Inténtalo de nuevo.",
  "auth.show": "Mostrar",
  "auth.hide": "Ocultar",
  "auth.passwordHint": "Mínimo 6 caracteres",
  "auth.usernameHint": "Letras, números, _ — mín 3 caracteres",

  "hunt.title": "ESTA SEMANA",
  "hunt.subtitle": "10 OBJETIVOS // ENCUÉNTRALOS",
  "hunt.empty": "NO HAY CAZA ACTIVA",
  "hunt.found": "ENCONTRADO",
  "hunt.progress": "PROGRESO",
  "hunt.points": "PUNTOS",
  "hunt.streak": "RACHA",
  "hunt.streakDays": "DÍAS",
  "hunt.daily": "BONUS DIARIO 2x",
  "hunt.dailyDesc": "Primera caza de hoy: ¡puntos DOBLES!",
  "hunt.combo": "COMBO",
  "hunt.comboDesc": "Caza 2 en 5 min: +50%",
  "hunt.timeLeft": "ACABA EN",
  "hunt.completed": "¡SEMANA COMPLETADA!",
  "hunt.rank": "TU RANGO",
  "hunt.others": "OTROS LO ENCONTRARON",
  "hunt.tap": "TOCA PARA CAZAR",

  "friends.title": "EL GRUPO",
  "friends.search": "BUSCA USUARIOS...",
  "friends.searchResults": "RESULTADOS",
  "friends.pending": "SOLICITUDES PENDIENTES",
  "friends.allies": "AMIGOS",
  "friends.alone": "CAZAS SOLO.",
  "friends.findFriends": "BUSCA AMIGOS.",
  "friends.add": "AÑADIR",
  "friends.weekPts": "PTS SEM",
  "friends.foundLabel": "ENCONTRADO:",
  "friends.requestSent": "ENVIADA",
  "friends.requestSentDesc": "Solicitud de amistad enviada.",
  "friends.accepted": "AMIGO AÑADIDO",
  "friends.acceptedDesc": "Ahora sois amigos.",

  "rankings.title": "RANKINGS",
  "rankings.local": "LOCAL",
  "rankings.global": "GLOBAL",
  "rankings.friends": "AMIGOS",
  "rankings.empty": "AÚN NO HAY DATOS",
  "rankings.foundShort": "HALLADOS",
  "rankings.you": "TÚ",

  "profile.title": "PERFIL",
  "profile.logout": "SALIR",
  "profile.totalPts": "PTS TOTALES",
  "profile.weekPoints": "PTS SEMANA",
  "profile.globalRank": "RANGO GLOBAL",
  "profile.weeklyMosaic": "MOSAICO SEMANAL",
  "profile.share": "COMPARTIR",
  "profile.noCaptures": "SIN CAPTURAS ESTA SEMANA",
  "profile.language": "IDIOMA",
  "profile.settings": "AJUSTES",
  "profile.editCity": "Ciudad",
  "profile.cityHint": "Se usa para el ranking local",
  "profile.saveSettings": "GUARDAR",
  "profile.saved": "¡Guardado!",
  "profile.saveError": "No se pudieron guardar los cambios",
  "profile.badges": "MEDALLAS",
  "profile.badge.first": "PRIMERA CAZA",
  "profile.badge.streak": "EN RACHA",
  "profile.badge.completer": "COMPLETISTA",
  "profile.badge.speedster": "VELOCISTA",
  "profile.badge.social": "SOCIABLE",
  "profile.locked": "BLOQUEADO",

  "camera.target": "OBJETIVO",
  "camera.color": "COLOR",
  "camera.frame": "ENCUADRA EL OBJETIVO. UN TIRO.",
  "camera.verified": "VERIFICADO POR IA",
  "camera.found": "OBJETIVO ENCONTRADO",
  "camera.confidence": "CONFIANZA",
  "camera.continue": "SEGUIR CAZANDO →",
  "camera.rejected": "LA IA LO HA RECHAZADO",
  "camera.notValid": "NO VÁLIDO",
  "camera.giveUp": "RENDIRSE",
  "camera.alreadyCaught": "YA HAS CAZADO ESTE OBJETIVO",
  "camera.cameraDenied": "ACCESO A LA CÁMARA DENEGADO. ACTÍVALO EN EL NAVEGADOR.",
  "camera.goBack": "VOLVER",
  "camera.aiActive": "VISIÓN IA ACTIVA",
  "camera.scan1": "ESCANEANDO IMAGEN...",
  "camera.scan2": "DETECTANDO OBJETOS...",
  "camera.scan3": "VERIFICANDO OBJETIVO...",
  "camera.scan4": "CALCULANDO CONFIANZA...",
  "camera.bonus": "¡BONUS DE VELOCIDAD!",
};

export const DICTS: Record<Lang, Dict> = { ca, es, en };

export function detectInitialLang(): Lang {
  if (typeof window === "undefined") return "ca";
  const stored = window.localStorage.getItem("flashhunt.lang");
  if (stored === "ca" || stored === "es" || stored === "en") return stored;
  const nav = (window.navigator.language ?? "ca").slice(0, 2).toLowerCase();
  if (nav === "ca") return "ca";
  if (nav === "es") return "es";
  return "en";
}
